...
/**
 * @title WWIIIGameV2
 * @notice Main game contract for WWIII blockchain game using eERC20 protocol with tour system
 * @dev Implements strategic warfare with encrypted token mechanics, ZK proofs, and tour-based gameplay
 */
contract WWIIIGameV2 is Ownable, ReentrancyGuard {
    ...
    enum GamePhase { ACTIVE, HALTED, ENDED }
    enum TourPhase { WAITING, DEPLOYMENT, BATTLE }
    
    /// @notice Current game phase
    GamePhase public gamePhase = GamePhase.ACTIVE;
    ...
    /// @notice Current tour configuration
    Tour public currentTour;
    
    /// @notice Current tour number (1-indexed)
    uint256 public tourNumber;
    ...
    /**
     * @notice Get current tour phase
     * @return Current phase of the tour system
     */
    ...
    /**
     * @notice Check if a player can retreat
     * @param player Player address to check
     * @return Whether the player can retreat
     */
    ...
    /**
     * @notice Get complete tour information including emissions array
     * @return deploymentEndTime When deployment phase ends
     * @return battleStartRound First round of tour
     * @return battleEndRound Last round of tour
     * @return roundEmissions Array of emission amounts for each round
     * @return phase Current tour phase
     */
    function getCurrentTourDetails() public view returns (
        uint256 deploymentEndTime,
        uint256 battleStartRound,
        uint256 battleEndRound,
        uint256[] memory roundEmissions,
        TourPhase phase
    ) {
        return (
            currentTour.deploymentEndTime,
            currentTour.battleStartRound,
            currentTour.battleEndRound,
            currentTour.roundEmissions,
            currentTour.phase
        );
    }
    ...
    /**
     * @notice Start a new tour with specified parameters
     * @param deploymentHours Duration of deployment phase (1-168 hours)
     * @param numberOfRounds Number of battle rounds in the tour
     * @param roundRewards Emission amount for each round
     */
    function startTour(
        ...
        if (currentTour.phase != TourPhase.WAITING) revert InvalidTourConfiguration();
        if (deploymentHours == 0 || deploymentHours > 168) revert InvalidTourConfiguration();
        if (numberOfRounds == 0 || numberOfRounds > 100) revert InvalidTourConfiguration();
        uint256 rewardsLength = roundRewards.length;
        if (rewardsLength != numberOfRounds) revert InvalidTourConfiguration();
        ...
        if (totalEmissions > emissionVault.remainingEmissions()) revert InvalidTourConfiguration();
        
        tourNumber++;
        currentTour = Tour({
            deploymentEndTime: block.timestamp + (deploymentHours * 1 hours),
    ...
    /**
     * @notice Get next round emission amount based on tour phase
     * @return Emission amount for next round or 0 if no tour active
     */
    function getNextEmissions() external view returns (uint256) {
        if (currentTour.phase == TourPhase.WAITING) {
            return 0; // No tour active
        }
        
        if (currentTour.phase == TourPhase.DEPLOYMENT) {
            // Return first round emission (what will be emitted when battle starts)
            return currentTour.roundEmissions[0];
        }
        
        uint256 roundIndex = currentRound - currentTour.battleStartRound;
        
        return currentTour.roundEmissions[roundIndex];
    }
